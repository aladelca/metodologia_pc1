"""
Tests para el módulo feature_builder de creación de características.
"""
...
class TestFeatureBuilder:
    ...
    def test_crear_categorias_edad(self, df_sample):
        ...
        # Verificar que se asignaron las categorías correctamente para las categorías que existen en los datos
    ...
    def test_calcular_ratio_salario_edad(self, df_sample):
        ...
        idx = 0
        expected_ratio = (df_sample.iloc[idx]['salario'] / df_sample.iloc[idx]['edad']).round(2)
    ...
    def test_crear_indice_satisfaccion(self, df_sample):
        ...
        columnas = ['satisfaccion_trabajo', 'satisfaccion_ambiente', 'satisfaccion_salario']
        pesos = [0.5, 0.3, 0.2]
        
        # Crear índice
        result = crear_indice_satisfaccion(df_sample, columnas_satisfaccion=columnas, pesos=pesos)
        ...
        idx = 0
        expected_index = (
            df_sample.iloc[idx]['satisfaccion_trabajo'] * 0.5 +
            df_sample.iloc[idx]['satisfaccion_ambiente'] * 0.3 +
            df_sample.iloc[idx]['satisfaccion_salario'] * 0.2
        ).round(2)
        assert result.iloc[idx]['indice_satisfaccion'] == expected_index
        
        # Verificar que los valores están en el rango esperado (1-5 para satisfacción normal)
    ...
    def test_crear_flags_riesgo(self, df_sample):
        ...
        for idx in range(len(result)):
            expected_score = (
                result.iloc[idx]['flag_baja_satisfaccion'] +
                result.iloc[idx]['flag_salario_bajo'] +
                result.iloc[idx]['flag_empleado_nuevo']
            )
    ...
    def test_seleccionar_mejores_caracteristicas(self, df_sample):
        ...
        # Verificar que el número de columnas es k + 1 (incluyendo la columna objetivo)
        ...
        # Verificar que todas las columnas del resultado existen en el DataFrame original
        for col in result.columns:
    ...
    def test_get_sample_data(self):
        ...
        expected_columns = [
            'id', 'edad', 'salario', 'antiguedad', 'departamento', 
            'ciudad', 'satisfaccion_trabajo', 'satisfaccion_ambiente', 
            'satisfaccion_salario', 'abandono'
        ]
